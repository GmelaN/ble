...
public:
  ...
  void ReceivedBroadcast (const Ptr<const Packet> packet, const Ptr<const BleNetDevice> netdevice);
...
private:
  ...
  int interval = 4*nNodes; //!< Time between two packets from the same node (for good results, should be larger than nNodes*nbConnInterval(s) 
...
BleTestCaseBC::ReceivedUnique (const Ptr<const Packet> packet)
{
	...
	void
BleTestCaseBC::ReceivedBroadcast (const Ptr<const Packet> packet, const Ptr<const BleNetDevice>  netdevice)
{
	Ptr<Packet> copy = packet->Copy();
	BleMacHeader header;
	copy->RemoveHeader(header);
  ...
  *m_stream->GetStream() << "Iteration, ID, transmitted, received, received unique, received error, broadcast received, TX Windows Skipped x coords, y coords " <<std::endl;
  for (uint8_t iterationI=0;iterationI<nbIterations;iterationI++){
   ...
   //helper.CreateAllLinks (bleNetDevices, scheduled, nbConnInterval);
    ...
    ApplicationContainer apps = helper.GenerateBroadcastTraffic (randT, bleDeviceNodes, pktsize, 0, packetSendDuration, interval);
    ...
    for (uint32_t i=0; i< bleNetDevices.GetN(); i++)
    {
      ...
      DynamicCast<BleNetDevice>(bleNetDevices.Get(i))->TraceConnectWithoutContext ("MacTx",MakeCallback(&BleTestCaseBC::Transmitted, this));
      DynamicCast<BleNetDevice>(bleNetDevices.Get(i))->TraceConnectWithoutContext ("MacRx",MakeCallback(&BleTestCaseBC::ReceivedUnique, this));
      DynamicCast<BleNetDevice>(bleNetDevices.Get(i))->TraceConnectWithoutContext ("MacRxBroadcast",MakeCallback(&BleTestCaseBC::ReceivedBroadcast, this));
      DynamicCast<BleNetDevice>(bleNetDevices.Get(i))->TraceConnectWithoutContext ("MacPromiscRx",MakeCallback(&BleTestCaseBC::Received, this));
      DynamicCast<BleNetDevice>(bleNetDevices.Get(i))->TraceConnectWithoutContext ("MacRxError",MakeCallback(&BleTestCaseBC::ReceivedError, this));
      DynamicCast<BleNetDevice>(bleNetDevices.Get(i))->TraceConnectWithoutContext ("TXWindowSkipped",MakeCallback(&BleTestCaseBC::TXWindowSkipped, this));
    }

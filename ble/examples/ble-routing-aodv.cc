...
NS_LOG_COMPONENT_DEFINE ("BleRoutingAodv");
  ...
  int nbIterations = 1;
  double length = 200; //<! Square room with length as distance
...
int main (int argc, char** argv)
{
  ...
  *m_stream->GetStream() << "Iteration, ID, transmitted, received, received unique, received error, broadcast received, TX Windows Skipped, x coords, y coords " <<std::endl;
  for (uint8_t iterationI=0;iterationI<nbIterations;iterationI++){
   ...
   // }
    ...
    aodv.Set("HelloInterval", TimeValue(Seconds(16)));
    aodv.Set("ActiveRouteTimeout", TimeValue(Seconds(480)));
    aodv.Set("BlackListTimeout", TimeValue(Seconds(488)));
    aodv.Set("NodeTraversalTime", TimeValue(Seconds(20)));
    aodv.Set("NextHopWait", TimeValue(Seconds(20)));
 ...
 //   interfaces.SetDefaultRouteInAllNodes (0);
    ...
    if (printRoutes)
    {
      ...
      aodv.PrintRoutingTableAllAt (Seconds (470), routingStream);
    }
